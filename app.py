...
def nueva_solicitud():
    if 'user_id' not in session:  # Cambia 'user_id' según cómo manejes tu sesión
        return redirect(url_for('login'))
        
    if request.method == 'POST':
        # Aquí irá el código para procesar el formulario y guardar el archivo (.stl/.dxf)
        pass

    # Esto busca y muestra el archivo HTML que creamos antes
    return render_template('nueva_solicitud.html')
